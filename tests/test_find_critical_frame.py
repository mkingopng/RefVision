...
def test_check_squat_depth_by_turnaround_pass():
	...
	def make_frame(hip_val: float) -> FrameResult:
		...
		"""
		boxes = DummyBoxes([[90, 90, 130, 130]])
		kpts = DummyKeypoints([
			[0, 0]] * 11 +  # padding
			[[100, hip_val],  # LEFT_HIP_IDX
			[120, hip_val],  # RIGHT_HIP_IDX
			[100, 120],  # LEFT_KNEE_IDX
			[120, 120],  # RIGHT_KNEE_IDX
			[0, 0],  # LEFT_ANKLE_IDX
			[0, 0]]  # RIGHT_ANKLE_IDX
		)
		return FrameResult(
...
def test_check_squat_depth_by_turnaround_fail() -> None:
	...
	def make_frame(hip_val: float, knee_val: float) -> FrameResult:
		...
		:return: A synthetic frame with keypoints and bounding boxes.
		"""
		boxes = DummyBoxes([[90, 90, 130, 130]])  # bounding box
		kpts = DummyKeypoints([
			[0, 0]] * 11 +  # padding
			[[100, hip_val],  # LEFT_HIP_IDX
			[120, hip_val],  # RIGHT_HIP_IDX
			[100, knee_val],  # LEFT_KNEE_IDX
			[120, knee_val],  # RIGHT_KNEE_IDX
			[0, 0],  # LEFT_ANKLE_IDX
			[0, 0]]  # RIGHT_ANKLE_IDX
		)
		return FrameResult(
